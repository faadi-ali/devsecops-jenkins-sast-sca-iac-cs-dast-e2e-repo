    ...
    stage('CompileandRunSonarAnalysis') {
      steps {
        withCredentials([string(credentialsId: 'SONAR_TOKEN', variable: 'SONAR_TOKEN')]) {
          bat("mvn -Dmaven.test.failure.ignore verify sonar:sonar -Dsonar.login=$devsecopsbuggywebapptoken -Dsonar.projectKey=devsecopsbuggywebapplication -Dsonar.host.url=https://sonarcloud.io/")
        }
      }
    }
    stage('Build') {
